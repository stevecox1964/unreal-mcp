...
export default function CharacterTab({ agentId }: { agentId: string }) {
  ...
  useEffect(() => {
    setStatus(null)
    api.getCharacter(agentId).then(setContent).catch(() => setStatus({ ok: false, msg: 'Failed to load' }))
  }, [agentId])
  ...
  const save = async () => {
    ...
    setStatus(null)
    try {
      await api.putCharacter(agentId, content)
