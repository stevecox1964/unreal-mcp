...
export default function RulesTab({ agentId }: { agentId: string }) {
  ...
  useEffect(() => {
    setStatus(null)
    api.getRules(agentId).then(setContent).catch(() => setStatus({ ok: false, msg: 'Failed to load' }))
  }, [agentId])
  ...
  const save = async () => {
    ...
    setStatus(null)
    try {
      await api.putRules(agentId, content)
