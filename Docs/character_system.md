...
> were **retired** along with the other editor-authoring tools (docs removed 2026-07-30). The
...
MCP Tool Call (Python)
  â†’ character_tools.py  â†’  send_command("command_name", {params})
...
| `unrealSIM.Build.cs` | Modified | Added `AIModule`, `NavigationSystem` |
| `Public/unrealSIMBridge.h` | Modified | Added include + `CharacterCommands` member |
| `Private/unrealSIMBridge.cpp` | Modified | Constructor init, destructor reset, dispatch block |
| `Python/unreal_sim_server.py` | Modified | Import + `register_character_tools(mcp)` |
...
| `Outbox` | `TArray<FString>` | Messages FROM the character â€” read via `get_character_messages`, written by `command_character_say` |
| `Memory` | `TMap<FString,FString>` | Key-value fact store â€” read/write via `set/get_character_memory` |
...
| `Health` | `float` | Health value â€” set this from your damage system |
...
| `OnSayRequested(Text)` | `command_character_say` is called â€” use this to drive your dialogue UI |

---

## Command Reference

All commands take `character_name` as the first parameter â€” the exact actor name in the level.
...
| `get_character_view(character_name)` | `get_character_view` | **STUB** â€” returns `not_implemented` |
| `get_nearby_actors(character_name, radius=500)` | `get_nearby_actors` | Actors within radius with distance |
| `get_heard_sounds(character_name)` | `get_heard_sounds` | **STUB** â€” returns `not_implemented` |
...
| `command_character_interact(character_name, target_actor="")` | `command_character_interact` | **STUB** â€” fires `OnInteractRequested` in Blueprint |
...
Currently fires `OnInteractRequested(target_actor)` on the component â€” implement the actual interaction logic in the NPC Blueprint.
...
# â†’ { "location": {"x": 100, "y": 200, "z": 0}, "rotation": {...} }
...
# â†’ { "key": "last_seen_player", "value": "north_gate" }
