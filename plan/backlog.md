...
- [ ] **APC map-view component** — an on-demand, APC-facing **top-down map** (distinct from the web
      **A1 `/map`**, which is the *human* observability view). Generated by **lizard brain** from what the
      APC knows: its current position + the destination place, plus the surrounding grid/place cells /
      known landmarks between them.
- [ ] **Charting a course** — the map is the substrate the APC uses to plot a route from *here* → *there*
      (feeds #6's lizard-brain routing; the LLM still decides whether to follow it).
